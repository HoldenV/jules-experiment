...
if __name__ == '__main__':
    ...
    if os.path.exists(config.TRADES_CSV_FILE): os.remove(config.TRADES_CSV_FILE)

    # Note: entry_order_id would be a string in real usage. Using datetime for entry_date.
    add_position("AAPL", 10, 150.00, "long", "test_order_id_aapl", datetime(2023, 10, 1))
    add_position("MSFT", 5, 300.00, "short", "test_order_id_msft", datetime(2023, 10, 5))
    positions = load_positions()
    print("Initial Positions:", positions)
    assert "AAPL" in positions and positions["AAPL"]["qty"] == 10 and isinstance(positions["AAPL"]["entry_date"], datetime)

    remove_position("AAPL", 155.00, "signal_exit", "test_exit_order_aapl")
    positions = load_positions()
    print("Positions after AAPL removal:", positions)
    assert "AAPL" not in positions and "MSFT" in positions

    # Simulate MSFT hitting max hold for check_and_manage_positions test
    # This requires mocking order_manager or having a dummy version for standalone test.
    # For now, this part of the test will mostly show logs.
    class DummyOrderManager:
        def place_limit_order(self, ticker, qty, limit_price, side, api_client):
            print(f"DUMMY_ORDER: Place {side} {qty} {ticker} @ {limit_price}")
            # Return a mock order object with an id
            class MockOrder: id = f"mock_exit_{ticker}"
            return MockOrder()
    order_manager = DummyOrderManager() # Override real order_manager for this test scope

    old_msft_pos_data = {
        "qty": 5, "entry_price": 300.00, "type": "short", "status": "open", "entry_order_id": "test_order_id_msft",
        "entry_date": (datetime.now() - timedelta(days=config.MAX_HOLDING_PERIOD_DAYS + 1)).isoformat() # Save as ISO string
    }
    save_positions({"MSFT": old_msft_pos_data})
    print(f"\nSimulating MSFT position older than {config.MAX_HOLDING_PERIOD_DAYS} days for management test.")

    current_dummy_prices = {"MSFT": 290.00}
    # For z-score test, a proper DataFrame would be needed. For max_hold, it's not strictly required.
    dummy_historical_data = {"MSFT": pd.DataFrame({'close': [290.00, 295.00]})} # Minimal data

    # Dummy API client for standalone test of check_and_manage_open_positions
    class DummyAPIClient: pass
    dummy_api = DummyAPIClient()

    check_and_manage_open_positions(current_dummy_prices, dummy_historical_data, api_client=dummy_api)
    positions_after_manage = load_positions()
    print("Positions after management cycle:", positions_after_manage)
    # Expected: MSFT status changed to 'pending_exit' due to max_hold
    if "MSFT" in positions_after_manage:
        assert positions_after_manage["MSFT"]["status"] == "pending_exit"
        print("MSFT position correctly marked as 'pending_exit'.")
    else:
        print("MSFT position missing after management cycle, check logic if not intended.")


    print("\nPDT Trade Count (placeholder):", get_pdt_trade_count())
    print("Available Cash (dummy API):", get_available_cash(api_client=dummy_api)) # Will use dummy API
